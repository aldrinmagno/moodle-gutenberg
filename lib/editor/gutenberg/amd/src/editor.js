...
define(['editor_gutenberg/gutenberg-editor'], function(GutenbergEditor) {
     ...
     */
    var setupForElementId = function(config) {
        var pendingKey = 'editor_gutenberg/editor:setupForElementId:' + config.elementId;
        ...
        var instance = GutenbergEditor.attachEditor(textarea, defaultEditorSettings);
        instanceMap.set(textarea, instance);

        // Ensure content is synced on form submit.
        // The onSaveContent callback already keeps textarea.value updated on every change,
        // but this handles edge cases where the form submits before a final React render.
        if (textarea.form) {
            textarea.form.addEventListener('submit', function() {
                // textarea.value is already kept in sync by onSaveContent.
            }, {once: false});
        }
